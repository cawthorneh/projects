...
(function () {
  ...
  function footLive(snap) {
    ...
    return "Inches of rain, averaged across <b>LCRA Hydromet</b> gauges in each area" +
           (w ? " · read at " + w : "") +
           ". The 48-hour figure is yesterday's full day plus today since midnight.";
  }

  function footStale(snap) {
    var w = whenText(snap);
    return "<b>These readings are behind.</b> The last successful read from LCRA Hydromet was" +
           (w ? " at " + w : " some time ago") + ", so treat them as out of date.";
  }

  function footDown() {
    return "<b>Readings unavailable.</b> The rainfall feed can't be reached right now — " +
           "these will fill in automatically once it responds.";
  }

  // Before the snapshot arrives there are no location names to show, so the
  // strip renders its own frame rather than inventing readings.
  var PLACEHOLDER_LABELS = ["Dripping Springs", "Austin", "Fredericksburg",
                            "Johnson City", "Blanco"];

  function placeholder() {
    return PLACEHOLDER_LABELS.map(function (l) { return { label: l, total: null }; });
  }

  function load(mount) {
    loadSnapshot(mount).then(function (snap) {
      var rows = snap.locations.map(function (l) {
        return { label: l.label, total: l.total48 };
      });
      var age = ageOf(snap);
