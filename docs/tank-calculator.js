...
(function () {
  ...
  var CSS = [
    ...
    '.drwt-cols{display:grid;grid-template-columns:minmax(0,340px) minmax(0,1fr);',
      'gap:1.15rem;align-items:start;margin-top:1.3rem}',
    ...
    '.drwt-rec{background:#3066AB;color:#fff;border-radius:16px;padding:1.35rem 1.4rem;',
      'box-shadow:0 10px 28px rgba(48,102,171,.18)}',
    ...
    '.drwt-rec-note{font-size:.84rem;color:#D6E4F2;margin-top:.7rem;line-height:1.5;max-width:52ch}',
  ...
  function results(r, v) {
    if (r.annualDemand <= 0) {
      return '<div class="drwt-rec">' +
        '<div class="drwt-rec-lab">Nothing to size yet</div>' +
        '<div class="drwt-rec-id is-small">Tell us what the water is for</div>' +
        '<div class="drwt-rec-note">With no household use, no irrigated area and no pool, ' +
        "there's no demand to size a tank against. Add an irrigated area or a pool on the " +
        "left, or switch to potable use.</div></div>" + fine(v);
    }
    var short = r.required === null;
    var rec = short
      ? '<div class="drwt-rec is-short">' +
          '<div class="drwt-rec-lab">Catchment is the limit</div>' +
          '<div class="drwt-rec-id is-small">More roof needed</div>' +
          '<div class="drwt-rec-range">' + gal(-r.balance) + " gal short each year</div>" +
      ...
      : '<div class="drwt-rec">' +
          ...
          '<div class="drwt-rec-range">' + r.tank.range + "</div>" +
    ...
    return rec + stats + chart(r) + fine(v);
  }
  ...
  function paint(root, v) {
    ...
    out.innerHTML = results(calculate(v), v);
    wireChart(out);
